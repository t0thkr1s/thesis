...
Az adatátviteli formátum egy olyan szöveges formátum, amit különböző platformok közötti adatcserére használnak.
...
A szakdolgozatomban is fontos szerepet tölt be, ugyanis a klien-szerver kommunikáció teljes egészében erre alapul.
...
\Aref{room}.~kódrészletben kiemeltem egy DAO interfészt.
...
Voltak olyanok is, amelyeket nagyon régóta nem voltak karbantartva és elavult kódbázissal rendelkeztek.
...
Amit szándékosan kihagytam, az a UI\footnote{User Interface (felhasználói felület)} tesztek felvétele.
...
Állíthatunk elvárásokat és interakciókat anélkül, hogy hozzáférnénk az alkalmazás nézeteihez és \texttt{Activity}-jeihez.
...
\Aref{espressotest}.~kódrészletben megfigyelhető, hogy egy \texttt{@Before} annotációval ellátott metódusban elmentettem egy teszt felhasználó nevet és jelszót.
Ezután pedig egyszerűen egy \texttt{Intent} segítségével elindítottam az \texttt{Activity}-t.
...
Az Espresso arra is nyújt megoldást, hogy ezeket az engedély kéréseket automatikusan elfogadjuk a tesztekben a \texttt{GrantPermissionRule} osztály segítségével.
...
Az Android platformot tekintve új komponensek érkeztek, ezek közé tartozik a \texttt{BottomAppBar} és a \texttt{Chip}.
